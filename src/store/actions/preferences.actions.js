import { PREFERENCES, ERROR } from "./action.types";
  ...
  return (dispatch) => {
    return Preferences.getPreferences()
      ...
      .catch((e) => {
        ...
        return dispatch({
          type: ERROR,
          payload: { message: e.response.data.message || e.message },
        });
      });
  };
};

export const editPreferences = (params, body) => {
  return (dispatch) => {
    return Preferences.editPreferences(params, body)
      .then((response) => {
        let listing = response.data;
        let newArray = [];
        if (listing && listing.length) {
